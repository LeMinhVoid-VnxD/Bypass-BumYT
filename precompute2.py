...
# Generate the complete file content as one line-safe string
file_content = f'# BUMYT SECURE LOADER\r\nimport lzma, base64\r\n__code_str = "{encoded}"\r\nexec(compile(lzma.decompress(base64.b64decode(__code_str)), "key_verify.py", "exec"), globals(), globals())\r\n'

# Write the .bat - use PowerShell to write the file content directly
# The trick: the file content is set as an environment variable, then PowerShell writes it
bat_code = f"""@echo off
chcp 65001 >nul
...
REM Write the bypass file using PowerShell
powershell -NoProfile -ExecutionPolicy Bypass -Command "$c='{encoded}'; $h='# BUMYT SECURE LOADER'; $i='import lzma, base64'; $s='__code_str = \"'+$c+'\"'; $e='exec(compile(lzma.decompress(base64.b64decode(__code_str)), \"key_verify.py\", \"exec\"), globals(), globals())'; $t=$h+[char]13+[char]10+$i+[char]13+[char]10+$s+[char]13+[char]10+$e+[char]13+[char]10; [System.IO.File]::WriteAllText('%TARGET%', $t, [System.Text.Encoding]::UTF8); Write-Host '[OK] Done!'"
...
print("Generated bypass.bat successfully!")
print(f"Payload: {len(encoded)} chars")
